...
public class SubjectKeywordServiceImpl extends BaseResourceService implements SubjectKeywordService {
	...
	public List<SubjectEntity> search(String question, int pageNo , int pageSize)  throws IllegalArgumentException, PersistLayerException {	
		...
		// STEP 4: calculate the hasCode for each term and query out the related keywordId
		...
		if (!keywords.isEmpty()) {
			List<Map<String,Object>> keywordsList = (List<Map<String,Object>>)getSqlMapClientTemplate().queryForList("subject.listKeywordsByHash", hashCodes.toArray(new Integer[0]));
		...
		if (!keywordIds.isEmpty()) {
		    ...
		    subjects = (List<SubjectEntity>)getSqlMapClientTemplate().queryForList("subject.listSubjectsByIds", sortedSubjectIds);
		    Collections.sort(subjects, new Comparator<SubjectEntity>(){
				@Override
				public int compare(SubjectEntity subject1, SubjectEntity subject2) {
					double d = scoresOfSubject.get(subject2.getId().intValue()) - scoresOfSubject.get(subject1.getId().intValue());
					if (d > 0) {
						return 1 ;
					}
					if (d < 0) {
	...
	protected Map<Integer,Double> searchScoredSubjectIds(Set<Integer> keywordIds) {
		...
		List<Map<String,Object>> subjectsList = (List<Map<String,Object>>)getSqlMapClientTemplate().queryForList("subject.listSubjectsByKeywordIds", keywordIds.toArray(new Integer[0]));
	...
	@Transactional
	public void seed(String subjectFile, String keywordSubjectFile) throws IllegalArgumentException, PersistLayerException, IOException {
	...
	public void seedKeywordSubjectFile(String keywordSubjectFile) throws FileNotFoundException, IOException {
		...
		try {
        	reader = getReader(keywordSubjectFile);
        	String strLine = null ;
        	...
        	do {        		
        		...
        		if (strLine != null) {
        			...
        			if (sep > 0) {
        				...
        				for (String subExtIdOccurPair: strLine.split(";")){
		...
		try {
        	...
        	do {
        		...
        		if (strLine != null) {
        			int sep = strLine.indexOf(':');
        			if (sep > 0) {
        				String sdx = strLine.substring(0,sep).trim();
        				String sub = strLine.substring(sep+1).trim();
        				if (!sdx.isEmpty() && !sub.isEmpty()) {
        				    ...
        				    try { 
        				    	...
        				    	logger.warn("illegal subjectId found from subjectLine:" + strLine);
        				    	continue;
        				    }
	...
	private BufferedReader getReader(String theFile)
			throws FileNotFoundException {
		BufferedReader reader;
		if (new File(theFile).exists()) {
			reader = new BufferedReader(new FileReader(theFile));
		} else {
			InputStream stream = getClass().getResourceAsStream("/" + theFile);
	...
	protected Map<Long,Long> getSubjectIdMap() { 
		...
		@SuppressWarnings("unchecked")
		List<Map<String,Integer>> list = (List<Map<String,Integer>>)getSqlMapClientTemplate().queryForList("subject.listSubjectExternalId");
		for (Map<String,Integer> map : list) {
			subExtToIdMap.put(map.get("external_id").longValue(), map.get("id").longValue());
		}
		list.clear(); 
	...
	protected void subjectFound(List<SubjectEntity> batchedEntities) {
		batchUpdateEntities(batchedEntities.toArray(new SubjectEntity[0]),"subject.addOrUpdateSubject" , batchedEntities.size());
	}

	protected Long keywordFound(KeywordEntity keywordEntity) {
		return (Long)getSqlMapClientTemplate().insert("subject.addOrUpdateKeyword",keywordEntity);
	}
	
	protected void subjectKeywordFound(List<SubjectKeyword> batchedSubjectKeyword) {
		batchUpdateEntities(batchedSubjectKeyword.toArray(new SubjectKeyword[0]), "subject.addOrUpdateSubjectKeyword", batchedSubjectKeyword.size());
	}
}
